...
merge.reads <- function(readgroup, ref.aa.seq = NULL, minInformation = 1.0, minReads = 0, processors = NULL, genetic.code = GENETIC_CODE){
    ...
    if(is.null(names(fwd.reads))){
        names(fwd.reads) = paste("fwd", 1:length(fwd.reads), sep = "_")
    }

    if(is.null(names(rev.reads))){
        names(rev.reads) = paste("rev", 1:length(rev.reads), sep = "_")
    }

    # Reverse complement reverse reads, and join into DNAStringSet
    print("Reverse compelementing reverse reads")
    fwd = mclapply(fwd.reads, get.primaryseq.xstring, revcomp = FALSE, mc.cores = processors) 
    rev = mclapply(rev.reads, get.primaryseq.xstring, revcomp = TRUE, mc.cores = processors) 
    reads = DNAStringSet(c(fwd, rev))
    ...
    if(!is.null(ref.aa.seq)) {
        ...
        corrected = CorrectFrameshifts(myXStringSet = reads, myAAStringSet = AAStringSet(ref.aa.seq), geneticCode = genetic.code, type = 'both', processors = processors, verbose = FALSE)
    ...
    if(!is.null(ref.aa.seq)){
        aln = AlignTranslation(reads, geneticCode = genetic.code, processors = processors, verbose = FALSE)
    }else{
        aln = AlignSeqs(reads, processors = processors, verbose = FALSE)
    }
    names(aln) = c(names(fwd.reads), names(rev.reads))
